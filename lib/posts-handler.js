...
const oneTimeTokenMap = new Map(); //キー：ユーザー名、値：トークン のワンタイムトークン（CSRFトークン）
  ...
  switch(req.method){
    case 'GET':
      ...
      Post.findAll({
        ...
        //サイトの/postsにアクセスするたびにフォームに埋め込むためのワンタイムトークンを用意しフォームに渡す
        const oneTimeToken = crypto.randomBytes(8).toString('hex'); //推測されにくいランダムなバイト列を作成し、それを 16 進数にした文字列を取得。トークンとして使う
        oneTimeTokenMap.set(req.user, oneTimeToken);                //現在のログインユーザーをキーとしてワンタイムトークンをセットする
        //全ての投稿内容を再描画
    ...
    case 'POST':
      req.on('data',(data)=>{
        ...
        console.log(decoded);  //フォームから送られてくるcontentとoneTimeTokenはこんな感じの文字列。& でデータが区切られてる感じ。
                               //=> content=テスト投稿&oneTimeToken=0cf7ef99b5bd99e8
        ...
        if(oneTimeTokenMap.get(req.user) === requestedOneTimeToken ){  //フォームから送られてきたワンタイムトークンが、事前に（/postsアクセス時に）用意したトークンと同じなら
          console.info('投稿されました: ' + content);                    //記事を投稿する
          Post.create({
            content: content,
            trackingCookie: trackingId,
            postedBy: req.user
          }).then(()=>{
            oneTimeTokenMap.delete(req.user);                         //ワンタイムトークンの削除
            handleRedirectPosts(req, res);                            //投稿してdb保存後は一覧へリダイレクト
          });
        } else {                                                      //トークンが不一致の場合、外部のサイトから投稿処理などをされてる可能性があるので例外ページに飛ばす
...
function handleDelete(req, res) {
  req.setEncoding('utf8');                                               //setEncodingを使えば、dataイベントが提供するチャンクがBufferオブジェクト（バイト配列）からutf8になるので decodeURIComponentが不要になる
  switch (req.method) {
    case 'POST':
      req.on('data', (data) => {
        //const decoded = decodeURIComponent(data);
        ...
        if(oneTimeTokenMap.get(req.user) !== requestedOneTimeToken ){    //ワンタイムトークンが不正なら
          return util.handleBadRequest(req, res);                        //例外ページに飛ばして終了
        }
        Post.findById(id).then((post) => {                               //投稿データが取得できたら
          if ( req.user === post.postedBy || req.user === 'admin' ) {    //basic認証のユーザー名と投稿データのユーザー名が一緒、もしくは管理人なら　（※）
            post.destroy().then(()=>{                                    //投稿を削除
              oneTimeTokenMap.delete(req.user);                          //ワンタイムトークンの削除
              ...
              handleRedirectPosts(req, res);                             //削除後は一覧へリダイレクト
            });
          }
        });
      });
